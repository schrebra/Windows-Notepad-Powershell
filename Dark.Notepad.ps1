...
# Font clamps for zoom (prevents degenerate font sizes)
...
# Dark theme palette
 ...
 $script:COLOR_BTN_HOVER  = "#505057"
 $script:COLOR_TITLE_BAR  = "#1E1E1E"
 $script:COLOR_OVERLAY    = "#3E3E42"
...
# Runtime globals
 ...
 $script:StatusUpdateTimer   = $null
 $script:AppIcon             = $null
 $script:FontCache           = $null      # HashSet<string> for fast font lookups
 $script:BrushCache          = @{}        # color hex -> Brush
 $script:DarkButtonStyle     = $null      # cached XAML-parsed Style
#endregion
...
function script:ExtractNotepadIcon {
    $paths = @(
        ...
        (Join-Path ${env:ProgramFiles} "Windows NT\Accessories\notepad.exe")
    )
...
function script:ApplyIcon([System.Windows.Window]$w) {
    if ($null -eq $script:AppIcon) { return }
    try {
        if ($null -ne $w) { $w.Icon = $script:AppIcon }
        if ($null -ne $script:UI.imgAppIcon) { $script:UI.imgAppIcon.Source = $script:AppIcon }
    } catch {}
}
#endregion
...
#region ==================== STATE ====================
 $script:State = @{
    ...
    LineEnding       = "CRLF"
    Encoding         = "UTF-8"
    ZoomLevel        = $script:DEF_ZOOM
    BaseFontSize     = [double]$script:DEF_FONT_SIZE
    BaseLineSpacing  = [double]$script:DEF_SPACING
 ...
 $script:Window             = $null
 $script:EditorScrollViewer = $null
#endregion
...
function script:GetFontCache {
    if ($null -eq $script:FontCache) {
        $script:FontCache = [System.Collections.Generic.HashSet[string]]::new(
            [System.StringComparer]::OrdinalIgnoreCase
        )
        try {
            foreach ($f in [System.Windows.Media.Fonts]::SystemFontFamilies) {
                $null = $script:FontCache.Add($f.Source)
            }
        } catch {}
    }
    return $script:FontCache
}

function script:TestFont([string]$Name) {
    if ([string]::IsNullOrWhiteSpace($Name)) { return $false }
    return (GetFontCache).Contains($Name)
}
...
function script:GetBrush([string]$hex) {
    ...
    if ($null -eq $b) {
        try {
            $b = [System.Windows.Media.BrushConverter]::new().ConvertFrom($hex)
            $b.Freeze()
...
function script:EolDisplayLabel([string]$Eol) {
    switch ($Eol) {
        "CRLF" { return "Windows (CRLF)"  }
        "LF"   { return "Unix (LF)"       }
        "CR"   { return "Macintosh (CR)"  }
        default { return "Windows (CRLF)" }
    }
}

function script:GetScrollViewer([System.Windows.DependencyObject]$d) {
    if ($null -eq $d) { return $null }
    if ($d -is [System.Windows.Controls.ScrollViewer]) { return $d }
    $count = [System.Windows.Media.VisualTreeHelper]::GetChildrenCount($d)
            ...
            <ControlTemplate TargetType="Button">
                ...
                <ControlTemplate.Triggers>
                    <Trigger Property="IsMouseOver" Value="True">
                        <Setter TargetName="bd" Property="Background" Value="#505057"/>
                        <Setter TargetName="bd" Property="BorderBrush" Value="#6A6A72"/>
                    </Trigger>
...
"@
    $script:DarkButtonStyle = [System.Windows.Markup.XamlReader]::Parse($xaml)
    return $script:DarkButtonStyle
}

function script:ApplyDarkDialogChrome([System.Windows.Window]$d) {
    $d.WindowStyle  = "None"
...
function script:BuildDarkDialog([System.Windows.Window]$d, [System.Windows.FrameworkElement]$content) {
    ApplyDarkDialogChrome $d

    $mainBorder = [System.Windows.Controls.Border]::new()
    $mainBorder.BorderBrush     = GetBrush $script:COLOR_BORDER
    $mainBorder.BorderThickness = [System.Windows.Thickness]::new(1)
    $mainBorder.Background      = GetBrush $script:COLOR_CHROME

    $root = [System.Windows.Controls.Grid]::new()
    $r0 = [System.Windows.Controls.RowDefinition]::new()
    $r0.Height = [System.Windows.GridLength]::new(32)
    $r1 = [System.Windows.Controls.RowDefinition]::new()
    $r1.Height = [System.Windows.GridLength]::new(1, [System.Windows.GridUnitType]::Star)
    $null = $root.RowDefinitions.Add($r0)
    $null = $root.RowDefinitions.Add($r1)

    $tb = [System.Windows.Controls.Grid]::new()
    $tb.Background = GetBrush $script:COLOR_EDITOR_BG
    [System.Windows.Controls.Grid]::SetRow($tb, 0)

    $lblTitle = [System.Windows.Controls.TextBlock]::new()
    $lblTitle.Text             = $d.Title
    $lblTitle.Foreground       = GetBrush $script:COLOR_EDITOR_FG
    $lblTitle.VerticalAlignment = "Center"
    $lblTitle.Margin           = [System.Windows.Thickness]::new(10, 0, 0, 0)
    $null = $tb.Children.Add($lblTitle)
    ...
    $btnClose.Style         = $script:Window.FindResource("TitleBarCloseButton")
    [System.Windows.Shell.WindowChrome]::SetIsHitTestVisibleInChrome($btnClose, $true)
    $btnClose.Add_Click({ $d.Close() })
    $null = $tb.Children.Add($btnClose)
    $null = $root.Children.Add($tb)

    [System.Windows.Controls.Grid]::SetRow($content, 1)
    $null = $root.Children.Add($content)

    $mainBorder.Child = $root
    $d.Content = $mainBorder
}

# Helper to style a dark-themed control (TextBox/Button/Label) consistently
function script:ApplyDarkControlTheme {
    param(
        [Parameter(Mandatory)] $Control,
        [string]$Type = "TextBox"
    )
    $Control.Background = GetBrush $script:COLOR_EDITOR_BG
    $Control.Foreground = GetBrush $script:COLOR_EDITOR_FG
    $Control.BorderBrush = GetBrush $script:COLOR_BORDER_LIT
    if ($Control -is [System.Windows.Controls.TextBox]) {
        $Control.CaretBrush = GetBrush $script:COLOR_EDITOR_FG
    }
}
#endregion
...
function script:DetectEncoding([string]$FilePath) {
    ...
    try {
        ...
        # BOM-based detection (highest confidence)
        if ($len -ge 4 -and $bytes[0] -eq 0x00 -and $bytes[1] -eq 0x00 -and $bytes[2] -eq 0xFE -and $bytes[3] -eq 0xFF) {
            return @{ Encoding = [System.Text.UTF32Encoding]::new($true, $true);  Name = "UTF-32 BE (BOM)"; BOM = $true }
        }
        if ($len -ge 4 -and $bytes[0] -eq 0xFF -and $bytes[1] -eq 0xFE -and $bytes[2] -eq 0x00 -and $bytes[3] -eq 0x00) {
            return @{ Encoding = [System.Text.UTF32Encoding]::new($false, $true); Name = "UTF-32 LE (BOM)"; BOM = $true }
        }
        if ($len -ge 3 -and $bytes[0] -eq 0xEF -and $bytes[1] -eq 0xBB -and $bytes[2] -eq 0xBF) {
            return @{ Encoding = [System.Text.UTF8Encoding]::new($true);  Name = "UTF-8 (BOM)";  BOM = $true }
        }
        if ($len -ge 2 -and $bytes[0] -eq 0xFE -and $bytes[1] -eq 0xFF) {
            return @{ Encoding = [System.Text.UnicodeEncoding]::new($true, $true);  Name = "UTF-16 BE (BOM)"; BOM = $true }
        }
        if ($len -ge 2 -and $bytes[0] -eq 0xFF -and $bytes[1] -eq 0xFE) {
            return @{ Encoding = [System.Text.UnicodeEncoding]::new($false, $true); Name = "UTF-16 LE (BOM)"; BOM = $true }
        }

        # Heuristic detection for no-BOM UTF-16
        if ($len -ge 2) {
            $nullOdd = 0; $nullEven = 0
            $check = [Math]::Min($len, 4096)
            for ($i = 0; $i -lt $check; $i++) {
                if ($bytes[$i] -eq 0x00) {
                    if (($i % 2) -eq 0) { $nullEven++ } else { $nullOdd++ }
                }
            }
            $halfCheck = $check / 4
            if ($nullOdd -gt $halfCheck -and $nullEven -lt ($halfCheck / 4)) {
                return @{ Encoding = [System.Text.UnicodeEncoding]::new($false, $false); Name = "UTF-16 LE (no BOM)"; BOM = $false }
            }
            if ($nullEven -gt $halfCheck -and $nullOdd -lt ($halfCheck / 4)) {
                return @{ Encoding = [System.Text.UnicodeEncoding]::new($true, $false);  Name = "UTF-16 BE (no BOM)"; BOM = $false }
            }
        }
        ...
        $isValidUtf8 = $true
        $i = 0
        $check = [Math]::Min($len, 8192)
        while ($i -lt $check) {
            $b = $bytes[$i]
            if ($b -le 0x7F) { $i++; continue }
            if (($b -band 0xE0) -eq 0xC0) { $seq = 1 }
            elseif (($b -band 0xF0) -eq 0xE0) { $seq = 2 }
            elseif (($b -band 0xF8) -eq 0xF0) { $seq = 3 }
            else { $isValidUtf8 = $false; break }
            for ($j = 0; $j -lt $seq; $j++) {
                $i++
                if ($i -ge $check -or ($bytes[$i] -band 0xC0) -ne 0x80) { $isValidUtf8 = $false; break }
            }
            if (-not $isValidUtf8) { break }
            $i++
        }
...
function script:DetectLineEnding([string]$Text) {
    ...
    $crlf = 0; $lf = 0; $cr = 0; $i = 0
    $tlen = $Text.Length
    while ($i -lt $tlen) {
        $c = $Text[$i]
        if ($c -eq "`r") {
            if (($i + 1) -lt $tlen -and $Text[$i + 1] -eq "`n") { $crlf++; $i += 2 }
            else { $cr++; $i++ }
        } elseif ($c -eq "`n") {
            $lf++; $i++
        } else {
...
function script:ConvertLineEndings([string]$Text, [string]$Target) {
    ...
    # Single normalization pass, then target conversion (less garbage than chained Replace)
    $norm = $Text -replace "`r`n", "`n" -replace "`r", "`n"
    switch ($Target) {
        "CRLF" { return $norm -replace "`n", "`r`n" }
        "LF"   { return $norm }
        "CR"   { return $norm -replace "`n", "`r" }
        default { return $norm -replace "`n", "`r`n" }
    }
}
#endregion
...
function script:WriteIni([string]$Path, [hashtable]$Data) {
    ...
    try {
        ...
        $null = $sb.AppendLine()
        # Iterate dynamically over all known sections (no hardcoding)
...
function script:SaveSettings {
    try {
        $rb = $script:Window.RestoreBounds
        $w = [int]$rb.Width
        $h = [int]$rb.Height
        if ($w -le 0) { $w = [int]$script:Window.ActualWidth }
        if ($h -le 0) { $h = [int]$script:Window.ActualHeight }
        $script:Settings.Window.Width  = "$w"
        $script:Settings.Window.Height = "$h"

        $script:Settings.Font.Family    = $script:UI.txtEditor.FontFamily.Source
        $script:Settings.Font.Size      = "$([int]$script:State.BaseFontSize)"
        ...
        $script:Settings.Editor.MarginLeft  = "$([int]$script:UI.txtEditor.Margin.Left)"
        $script:Settings.Editor.MarginRight = "$([int]$script:UI.txtEditor.Margin.Right)"
...
#region ==================== ZOOM ====================
function script:UpdateZoomMenuState {
    try { $script:UI.mnuZoomReset.IsEnabled = ($script:State.ZoomLevel -ne $script:DEF_ZOOM) } catch {}
}

function script:ApplyZoom {
    $factor  = [double]$script:State.ZoomLevel / 100.0
    $newSize = Clamp ([double]$script:State.BaseFontSize * $factor) $script:ZOOM_FONT_MIN $script:ZOOM_FONT_MAX
...
function script:ZoomReset {
    if ($script:State.ZoomLevel -eq $script:DEF_ZOOM) { return }
    $script:State.ZoomLevel = $script:DEF_ZOOM
    ApplyZoom
}

function script:UpdateZoomDisplay {
    try { $script:UI.txtZoom.Text = "$($script:State.ZoomLevel)%" } catch {}
}
#endregion
...
<Window x:Name="mainWindow"
        ...
        MinWidth="$script:MIN_WINDOW_W" MinHeight="$script:MIN_WINDOW_H"
        Background="$script:COLOR_CHROME">

    <WindowChrome.WindowChrome>
        <WindowChrome CaptionHeight="32" ResizeBorderThickness="6" CornerRadius="0"
    ...
    <Window.Resources>
            ...
            <Setter Property="Template">
                <Setter.Value>
                    <ControlTemplate TargetType="Thumb">
                        <Border Background="#4D4D4D" BorderBrush="#1E1E1E" BorderThickness="1"/>
                    </ControlTemplate>
                </Setter.Value>
            </Setter>
        </Style>
                ...
                <Setter.Value>
                    <ControlTemplate TargetType="ScrollBar">
                        <Grid Background="{TemplateBinding Background}">
                            ...
                            <RepeatButton Grid.Row="0" Style="{StaticResource ScrollBarButton}" Command="ScrollBar.LineUpCommand">
                                <Path Data="M 2,5 L 5,2 L 8,5" Stroke="#999999" StrokeThickness="1.5"/>
                            </RepeatButton>
                            ...
                            <RepeatButton Grid.Row="2" Style="{StaticResource ScrollBarButton}" Command="ScrollBar.LineDownCommand">
                                <Path Data="M 2,2 L 5,5 L 8,2" Stroke="#999999" StrokeThickness="1.5"/>
                            </RepeatButton>
                        </Grid>
                    </ControlTemplate>
                </Setter.Value>
                        ...
                        <Setter.Value>
                            <ControlTemplate TargetType="ScrollBar">
                                <Grid Background="{TemplateBinding Background}">
                                    ...
                                    <RepeatButton Grid.Column="0" Style="{StaticResource ScrollBarButton}" Command="ScrollBar.LineLeftCommand">
                                        <Path Data="M 5,2 L 2,5 L 5,8" Stroke="#999999" StrokeThickness="1.5"/>
                                    </RepeatButton>
                                    ...
                                    <RepeatButton Grid.Column="2" Style="{StaticResource ScrollBarButton}" Command="ScrollBar.LineRightCommand">
                                        <Path Data="M 2,2 L 5,5 L 2,8" Stroke="#999999" StrokeThickness="1.5"/>
                                    </RepeatButton>
                                </Grid>
                            </ControlTemplate>
                        </Setter.Value>
            ...
            <Setter Property="Template">
                <Setter.Value>
                    <ControlTemplate TargetType="ContextMenu">
                        <Border Background="#2D2D30" BorderBrush="#3F3F46" BorderThickness="1" Padding="0,2">
            ...
            <Setter Property="Template">
                <Setter.Value>
                    <ControlTemplate TargetType="Separator">
                        <Rectangle Height="1" Fill="#3F3F46" Margin="25,3,5,3"/>
                    </ControlTemplate>
                </Setter.Value>
            </Setter>
        </Style>
        ...
        <ControlTemplate x:Key="TopLevelHeaderTemplate" TargetType="MenuItem">
            <Border x:Name="Border" Background="Transparent" SnapsToDevicePixels="True">
                <Grid>
                    <ContentPresenter Margin="{TemplateBinding Padding}" RecognizesAccessKey="True" ContentSource="Header" VerticalAlignment="Center"/>
                    <Popup x:Name="Popup" Placement="Bottom" IsOpen="{TemplateBinding IsSubmenuOpen}" AllowsTransparency="True" Focusable="False">
                        <Border Background="#2D2D30" BorderBrush="#3F3F46" BorderThickness="1">
                            <ItemsPresenter KeyboardNavigation.DirectionalNavigation="Cycle" Margin="0,2"/>
                        </Border>
                    </Popup>
                </Grid>
            </Border>
        ...
        <ControlTemplate x:Key="SubmenuItemTemplate" TargetType="MenuItem">
            <Border x:Name="Border" Background="{TemplateBinding Background}" SnapsToDevicePixels="True">
                <Grid>
                    ...
                    <Path x:Name="CheckMark" Visibility="Hidden" Grid.Column="0" Margin="5,0" VerticalAlignment="Center" HorizontalAlignment="Center" Data="M 0,4 L 3,7 L 8,0" Stroke="#D4D4D4" StrokeThickness="2"/>
                    ...
                    <TextBlock Grid.Column="2" x:Name="InputGestureText" Margin="20,0,10,0" Text="{TemplateBinding InputGestureText}" VerticalAlignment="Center" Foreground="#888888"/>
                    <Path Grid.Column="3" x:Name="RightArrow" Visibility="Hidden" Margin="0,0,5,0" VerticalAlignment="Center" HorizontalAlignment="Right" Data="M 0,0 L 4,3 L 0,6 Z" Fill="#D4D4D4"/>
                    <Popup x:Name="Popup" Placement="Right" HorizontalOffset="0" VerticalOffset="-2" IsOpen="{TemplateBinding IsSubmenuOpen}" AllowsTransparency="True" Focusable="False">
                        <Border x:Name="SubmenuBorder" Background="#2D2D30" BorderBrush="#3F3F46" BorderThickness="1">
    ...
    <Grid x:Name="RootGrid">
        ...
        <Grid Grid.Row="0" Background="#1E1E1E">
            ...
            <Viewbox Grid.Column="0" Width="14" Height="14" Margin="10,0,5,0" VerticalAlignment="Center">
                <Canvas Width="16" Height="16">
                    <Path Data="M2,0 L10,0 L14,4 L14,16 L2,16 Z" Stroke="#D4D4D4" StrokeThickness="1.5" Fill="Transparent"/>
                    <Path Data="M10,0 L10,4 L14,4" Stroke="#D4D4D4" StrokeThickness="1.5" Fill="Transparent"/>
                    <Path Data="M4,7 L10,7 M4,10 L12,10 M4,13 L9,13" Stroke="#D4D4D4" StrokeThickness="1.5" StrokeStartLineCap="Round" StrokeEndLineCap="Round"/>
                </Canvas>
            </Viewbox>
            ...
            <StackPanel Grid.Column="2" Orientation="Horizontal" WindowChrome.IsHitTestVisibleInChrome="True">
                ...
                <Button Name="btnMax" Style="{StaticResource TitleBarButton}">
                    <Grid Width="12" Height="12">
                        <Path Data="M 0.5,0.5 L 9.5,0.5 L 9.5,9.5 L 0.5,9.5 Z" Stroke="{Binding Foreground, RelativeSource={RelativeSource AncestorType=Button}}" StrokeThickness="1" Fill="Transparent" SnapsToDevicePixels="True">
                            <Path.Style>
                        ...
                        <Grid>
                            ...
                            <Path Data="M 0.5,0.5 L 7.5,0.5 L 7.5,7.5 L 0.5,7.5 Z" Stroke="{Binding Foreground, RelativeSource={RelativeSource AncestorType=Button}}" StrokeThickness="1" Fill="Transparent" SnapsToDevicePixels="True" Margin="3,0,0,3"/>
                            <Path Data="M 0.5,0.5 L 7.5,0.5 L 7.5,7.5 L 0.5,7.5 Z" Stroke="{Binding Foreground, RelativeSource={RelativeSource AncestorType=Button}}" StrokeThickness="1" Fill="{Binding Background, RelativeSource={RelativeSource AncestorType=Button}}" SnapsToDevicePixels="True" Margin="0,3,3,0"/>
                        </Grid>
                    </Grid>
                </Button>
        ...
        <Border Grid.Row="1" BorderBrush="#3F3F46" BorderThickness="0,0,0,1">
            <Menu Name="menuBar" Padding="4,0,0,0">
                ...
                <MenuItem Header="F_ormat">
                    ...
                    <MenuItem Header="_Line Endings" Name="mnuLineEndings">
                        <MenuItem Header="Windows (CRLF)" Name="mnuEolCRLF" ToolTip="Carriage Return + Line Feed."/>
                        <MenuItem Header="Unix (LF)" Name="mnuEolLF" ToolTip="Line Feed only."/>
                        <MenuItem Header="Macintosh (CR)" Name="mnuEolCR" ToolTip="Carriage Return only."/>
                    </MenuItem>
        ...
        <Border Grid.Row="2" Background="$script:COLOR_CHROME">
            <TextBox Name="txtEditor" AcceptsReturn="True" AcceptsTab="True"
                     ...
                     BorderThickness="0"
                     Background="$script:COLOR_EDITOR_BG" Foreground="$script:COLOR_EDITOR_FG"
                     Padding="12,10" UndoLimit="1000" IsUndoEnabled="True"
                     CaretBrush="$script:COLOR_EDITOR_FG"
                     ScrollViewer.CanContentScroll="False">
                <TextBox.ContextMenu>
                    <ContextMenu FontFamily="Segoe UI" FontSize="12">
                        <MenuItem Name="ctxCut" Header="Cu_t" InputGestureText="Ctrl+X"/>
                        <MenuItem Name="ctxCopy" Header="_Copy" InputGestureText="Ctrl+C"/>
                        <MenuItem Name="ctxPaste" Header="_Paste" InputGestureText="Ctrl+V"/>
                        <Separator/>
                        <MenuItem Name="ctxSelAll" Header="Select _All" InputGestureText="Ctrl+A"/>
                    </ContextMenu>
                </TextBox.ContextMenu>
            </TextBox>
        </Border>

        <Border Grid.Row="3" Name="statusBorder" BorderBrush="#3F3F46" BorderThickness="0,1,0,0">
            <Border BorderBrush="#1E1E1E" BorderThickness="0,1,0,0">
                <StatusBar Name="statusBar" Background="#2D2D30" Foreground="#D4D4D4" Padding="6,1" Height="22">
...
try {
    ...
    $null = [System.Windows.MessageBox]::Show(
        "XAML load failed: $($_.Exception.Message)", "Fatal Error", "OK", "Error"
    )
    exit 1
}
...
ApplyIcon $script:Window
 ...
 $script:ControlNames = @(
    "RootGrid","txtEditor","txtPos","txtWords","txtChars","txtLines","txtEnc","txtEol","txtZoom",
    "statusBar","statusBorder","menuBar",
    ...
    "mnuLineEndings","mnuEolCRLF","mnuEolLF","mnuEolCR",
    "mnuEncoding","mnuEncUtf8","mnuEncUtf8Bom","mnuEncUtf16LE","mnuEncUtf16BE",
    "txtWindowTitle","btnMin","btnMax","btnClose",
    "ctxCut","ctxCopy","ctxPaste","ctxSelAll"
)
...
try {
    ...
    $script:State.BaseFontSize   = [double]$fsize
    $script:UI.txtEditor.FontSize = [double]$fsize
    ...
    $script:State.BaseLineSpacing = [double]$sp
    ApplyLineSpacingToEditor
...
function script:UpdateTitle {
    try {
        $name = if ([string]::IsNullOrEmpty($script:State.FilePath)) {
            "Untitled"
        } else {
            [System.IO.Path]::GetFileName($script:State.FilePath)
        }
        $mod = if ($script:State.IsModified) { "*" } else { "" }
        $fullTitle = "$mod$name - $script:APP_NAME"
        $script:Window.Title = $fullTitle
        if ($null -ne $script:UI.txtWindowTitle) {
            $script:UI.txtWindowTitle.Text = $fullTitle
        }
    } catch {}
}
...
function script:UpdateStatusImmediate {
    ...
    try {
        ...
        if ($len -eq 0) {
            ...
            # Use regex for faster word counting than char-by-char iteration
        ...
        $script:UI.txtEol.Text  = EolDisplayLabel $script:State.LineEnding
        $script:UI.txtEnc.Text  = $script:State.Encoding
        $script:UI.txtZoom.Text = "$($script:State.ZoomLevel)%"
    } catch {}
}

function script:RequestStatusUpdate {
    if (-not $script:StatusUpdatePending) {
        $script:StatusUpdatePending = $true
        $script:StatusUpdateTimer.Stop()
        $script:StatusUpdateTimer.Start()
    }
}
#endregion
...
function script:ConfirmSave {
    ...
    $r = [System.Windows.MessageBox]::Show(
        "Do you want to save changes to $name`?", $script:APP_NAME, "YesNoCancel", "Warning"
    )
    switch ($r) {
        "Yes"    { return (DoSave)    }
        "No"     { return $true       }
...
function script:DoNew {
    ...
    $script:State.LineEnding  = "CRLF"
    $script:State.Encoding    = "UTF-8"
    $script:State.ZoomLevel   = $script:DEF_ZOOM
    ApplyZoom
    UpdateTitle
    UpdateStatusImmediate
...
function script:DoOpen {
    ...
    try {
        ...
        if ($fi.Length -gt 10MB) {
            $r = [System.Windows.MessageBox]::Show(
                "File exceeds 10 MB. Loading may be slow. Continue?", "Large File", "YesNo", "Warning"
            )
            if ($r -ne "Yes") { return }
        }
        $det = DetectEncoding $dlg.FileName
        $script:State.Encoding = $det.Name
...
function script:GetCurrentEncoding {
    ...
    foreach ($key in $script:ENCODINGS.Keys) {
        if ($key -eq $name) { return $script:ENCODINGS[$key] }
    }
    switch -Wildcard ($name) {
        "*UTF-8*BOM*"   { return [System.Text.UTF8Encoding]::new($true)   }
        "*UTF-8*"       { return [System.Text.UTF8Encoding]::new($false)  }
        "*UTF-16 LE*"   { return [System.Text.UnicodeEncoding]::new($false, $true) }
        "*UTF-16 BE*"   { return [System.Text.UnicodeEncoding]::new($true, $true)  }
...
function script:NotFound([string]$Text) {
    $null = [System.Windows.MessageBox]::Show(
        "Cannot find `"$Text`"", $script:APP_NAME, "OK", "Information"
    )
}
...
function script:DoFind([string]$Text, [bool]$Case, [bool]$Msg = $true) {
    ...
    if ($idx -ge 0) {
        ...
        try {
            $script:UI.txtEditor.SelectionBrush = GetBrush $script:COLOR_FIND_BG
        } catch {}
...
function script:DoFindNext {
    if ([string]::IsNullOrWhiteSpace($script:State.FindText)) {
        ShowFindDlg
    } else {
        $null = DoFind $script:State.FindText $script:State.FindCase
    }
}

function script:NewDarkDialog([string]$Title, [double]$Width, [double]$Height) {
...
function script:NewDarkTextBox {
    $tb = [System.Windows.Controls.TextBox]::new()
    $tb.Height = 26
    $tb.VerticalContentAlignment = "Center"
    ApplyDarkControlTheme -Control $tb
    return $tb
}

function script:NewDarkButton([string]$Text, [double]$Width = 90, [double]$Height = 28) {
    $b = [System.Windows.Controls.Button]::new()
    $b.Content = $Text
    $b.Width = $Width
    $b.Height = $Height
    $b.Background  = GetBrush $script:COLOR_BTN_BG
    $b.Foreground  = GetBrush $script:COLOR_EDITOR_FG
...
function script:ShowFindDlg {
    ...
    $null = $g.RowDefinitions.Add([System.Windows.Controls.RowDefinition]::new())
    foreach ($cw in @("Auto", "1*", "Auto")) {
        $cd = [System.Windows.Controls.ColumnDefinition]::new()
        $cd.Width = if ($cw -eq "1*") {
            [System.Windows.GridLength]::new(1, "Star")
        } else {
    ...
    $lbl.Foreground = GetBrush $script:COLOR_EDITOR_FG
    [System.Windows.Controls.Grid]::SetRow($lbl, 0); [System.Windows.Controls.Grid]::SetColumn($lbl, 0)
    $null = $g.Children.Add($lbl)

    $tb = NewDarkTextBox
    $tb.Margin = [System.Windows.Thickness]::new(10, 0, 10, 0)
    $tb.Text = $script:State.FindText
    [System.Windows.Controls.Grid]::SetRow($tb, 0); [System.Windows.Controls.Grid]::SetColumn($tb, 1)
    $null = $g.Children.Add($tb)

    $bf = NewDarkButton "Find Next" 90 28
    $bf.IsDefault = $true
    [System.Windows.Controls.Grid]::SetRow($bf, 0); [System.Windows.Controls.Grid]::SetColumn($bf, 2)
    ...
    $ck.Foreground = GetBrush $script:COLOR_EDITOR_FG
    [System.Windows.Controls.Grid]::SetRow($ck, 1); [System.Windows.Controls.Grid]::SetColumn($ck, 1)
    $null = $g.Children.Add($ck)

    $bc = NewDarkButton "Cancel" 90 28
    $bc.IsCancel = $true
    $bc.Margin = [System.Windows.Thickness]::new(0, 15, 0, 0)
    [System.Windows.Controls.Grid]::SetRow($bc, 1); [System.Windows.Controls.Grid]::SetColumn($bc, 2)
...
function script:ShowReplaceDlg {
    $d = NewDarkDialog "Replace" 480 210
    ...
    $l1.Content = "Find what:"
    $l1.VerticalAlignment = "Center"
    $l1.Foreground = GetBrush $script:COLOR_EDITOR_FG
    [System.Windows.Controls.Grid]::SetRow($l1, 0); [System.Windows.Controls.Grid]::SetColumn($l1, 0)
    $null = $g.Children.Add($l1)

    $tFind = NewDarkTextBox
    $tFind.Margin = [System.Windows.Thickness]::new(10, 5, 10, 5)
    $tFind.Text = $script:State.FindText
    [System.Windows.Controls.Grid]::SetRow($tFind, 0); [System.Windows.Controls.Grid]::SetColumn($tFind, 1)
    $null = $g.Children.Add($tFind)

    $bfn = NewDarkButton "Find Next" 100 28
    $bfn.Margin = [System.Windows.Thickness]::new(0, 5, 0, 5)
    [System.Windows.Controls.Grid]::SetRow($bfn, 0); [System.Windows.Controls.Grid]::SetColumn($bfn, 2)
    $null = $g.Children.Add($bfn)

    $l2 = [System.Windows.Controls.Label]::new()
    $l2.Content = "Replace with:"
    $l2.VerticalAlignment = "Center"
    $l2.Foreground = GetBrush $script:COLOR_EDITOR_FG
    [System.Windows.Controls.Grid]::SetRow($l2, 1); [System.Windows.Controls.Grid]::SetColumn($l2, 0)
    $null = $g.Children.Add($l2)

    $tRepl = NewDarkTextBox
    $tRepl.Margin = [System.Windows.Thickness]::new(10, 5, 10, 5)
    [System.Windows.Controls.Grid]::SetRow($tRepl, 1); [System.Windows.Controls.Grid]::SetColumn($tRepl, 1)
    $null = $g.Children.Add($tRepl)

    $brp = NewDarkButton "Replace" 100 28
    $brp.Margin = [System.Windows.Thickness]::new(0, 5, 0, 5)
    [System.Windows.Controls.Grid]::SetRow($brp, 1); [System.Windows.Controls.Grid]::SetColumn($brp, 2)
    ...
    $ck.IsChecked = $script:State.FindCase
    $ck.Foreground = GetBrush $script:COLOR_EDITOR_FG
    [System.Windows.Controls.Grid]::SetRow($ck, 2); [System.Windows.Controls.Grid]::SetColumn($ck, 1)
    $null = $g.Children.Add($ck)

    $bra = NewDarkButton "Replace All" 100 28
    $bra.Margin = [System.Windows.Thickness]::new(0, 5, 0, 5)
    [System.Windows.Controls.Grid]::SetRow($bra, 2); [System.Windows.Controls.Grid]::SetColumn($bra, 2)
    $null = $g.Children.Add($bra)

    $bcx = NewDarkButton "Cancel" 100 28
    $bcx.IsCancel = $true
    $bcx.Margin = [System.Windows.Thickness]::new(0, 5, 0, 5)
    [System.Windows.Controls.Grid]::SetRow($bcx, 3); [System.Windows.Controls.Grid]::SetColumn($bcx, 2)
    ...
    $brp.Add_Click({
        ...
        if ($sel.Length -gt 0) {
            $match = if ($mc) {
                ...
                $sel.Equals($s, [System.StringComparison]::OrdinalIgnoreCase)
            }
...
function script:ShowFontDlg {
    ...
    try {
        $pt = [Math]::Round($script:State.BaseFontSize * 72 / 96)
    ...
    try {
        if ($dlg.ShowDialog() -eq "OK") {
            ...
            $script:State.BaseFontSize = [double]($dlg.Font.Size * 96 / 72)
            ApplyZoom
        }
    } finally {
        $dlg.Dispose()
...
function script:ShowEditorCfg {
    $d = NewDarkDialog "Editor Settings" 0 0
    $d.SizeToContent = "WidthAndHeight"

    $brFg  = GetBrush $script:COLOR_EDITOR_FG
    $brBdr = GetBrush $script:COLOR_BORDER_LIT

    $outerBorder = [System.Windows.Controls.Border]::new()
    $outerBorder.Padding = [System.Windows.Thickness]::new(14, 12, 14, 12)
    $outerBorder.MinWidth = 290
    $stack = [System.Windows.Controls.StackPanel]::new()
    $curMargin  = $script:UI.txtEditor.Margin
    $curSpacing = [double]$script:State.BaseLineSpacing

    # Margins group
    $mgb = [System.Windows.Controls.GroupBox]::new()
    $mgb.Header = " Margins (px) "
    $mgb.Padding = [System.Windows.Thickness]::new(8, 4, 8, 4)
    $mgb.Margin = [System.Windows.Thickness]::new(0, 0, 0, 6)
    $mgb.FontSize = 11
    $mgb.Foreground = $brFg
    $mgb.BorderThickness = [System.Windows.Thickness]::new(1)
    $mgb.BorderBrush = $brBdr

    $mg = [System.Windows.Controls.Grid]::new()
    $null = $mg.RowDefinitions.Add([System.Windows.Controls.RowDefinition]::new())
    $null = $mg.RowDefinitions.Add([System.Windows.Controls.RowDefinition]::new())
    $mc0 = [System.Windows.Controls.ColumnDefinition]::new(); $mc0.Width = [System.Windows.GridLength]::new(45)
    $mc1 = [System.Windows.Controls.ColumnDefinition]::new(); $mc1.Width = [System.Windows.GridLength]::new(1, "Star")
    $null = $mg.ColumnDefinitions.Add($mc0)
    $null = $mg.ColumnDefinitions.Add($mc1)

    $llbl = [System.Windows.Controls.Label]::new()
    $llbl.Content = "Left:"
    $llbl.VerticalAlignment = "Center"
    $llbl.FontSize = 11
    $llbl.Padding = [System.Windows.Thickness]::new(0)
    $llbl.Foreground = $brFg
    [System.Windows.Controls.Grid]::SetRow($llbl, 0); [System.Windows.Controls.Grid]::SetColumn($llbl, 0)
    $null = $mg.Children.Add($llbl)

    $tLeft = NewDarkTextBox
    $tLeft.Width = 60; $tLeft.Height = 20; $tLeft.FontSize = 11
    $tLeft.Margin = [System.Windows.Thickness]::new(0, 2, 0, 2)
    $tLeft.HorizontalAlignment = "Left"
    $tLeft.Padding = [System.Windows.Thickness]::new(3, 0, 3, 0)
    $tLeft.Text = "$([int]$curMargin.Left)"
    [System.Windows.Controls.Grid]::SetRow($tLeft, 0); [System.Windows.Controls.Grid]::SetColumn($tLeft, 1)
    $null = $mg.Children.Add($tLeft)

    $rlbl = [System.Windows.Controls.Label]::new()
    $rlbl.Content = "Right:"
    $rlbl.VerticalAlignment = "Center"
    $rlbl.FontSize = 11
    $rlbl.Padding = [System.Windows.Thickness]::new(0)
    $rlbl.Foreground = $brFg
    [System.Windows.Controls.Grid]::SetRow($rlbl, 1); [System.Windows.Controls.Grid]::SetColumn($rlbl, 0)
    $null = $mg.Children.Add($rlbl)

    $tRight = NewDarkTextBox
    $tRight.Width = 60; $tRight.Height = 20; $tRight.FontSize = 11
    $tRight.Margin = [System.Windows.Thickness]::new(0, 2, 0, 2)
    $tRight.HorizontalAlignment = "Left"
    $tRight.Padding = [System.Windows.Thickness]::new(3, 0, 3, 0)
    $tRight.Text = "$([int]$curMargin.Right)"
    [System.Windows.Controls.Grid]::SetRow($tRight, 1); [System.Windows.Controls.Grid]::SetColumn($tRight, 1)
    ...
    $sgb.Padding = [System.Windows.Thickness]::new(8, 4, 8, 4)
    $sgb.Margin = [System.Windows.Thickness]::new(0, 0, 0, 6)
    $sgb.FontSize = 11
    $sgb.Foreground = $brFg
    $sgb.BorderThickness = [System.Windows.Thickness]::new(1)
    $sgb.BorderBrush = $brBdr

    $sg = [System.Windows.Controls.StackPanel]::new()
    $sg.Orientation = "Horizontal"
    $slbl = [System.Windows.Controls.Label]::new()
    $slbl.Content = "Multiplier:"
    $slbl.VerticalAlignment = "Center"
    $slbl.FontSize = 11
    $slbl.Padding = [System.Windows.Thickness]::new(0)
    $slbl.Foreground = $brFg
    $null = $sg.Children.Add($slbl)

    $tSpace = NewDarkTextBox
    $tSpace.Width = 60; $tSpace.Height = 20; $tSpace.FontSize = 11
    $tSpace.Margin = [System.Windows.Thickness]::new(8, 0, 0, 0)
    $tSpace.Padding = [System.Windows.Thickness]::new(3, 0, 3, 0)
    ...
    $bp.HorizontalAlignment = "Left"
    $bp.Margin = [System.Windows.Thickness]::new(0, 6, 0, 0)

    $bOK = NewDarkButton "OK" 65 22
    $bOK.FontSize = 11
    $bOK.IsDefault = $true
    $bOK.Margin = [System.Windows.Thickness]::new(0, 0, 5, 0)
    $null = $bp.Children.Add($bOK)

    $bApply = NewDarkButton "Apply" 65 22
    $bApply.FontSize = 11
    $bApply.Margin = [System.Windows.Thickness]::new(0, 0, 5, 0)
    $null = $bp.Children.Add($bApply)

    $bCancel = NewDarkButton "Cancel" 65 22
    $bCancel.FontSize = 11
    ...
    $tryApply = {
        ...
        if ($lv -lt $script:MIN_MARGIN -or $lv -gt $script:MAX_MARGIN) {
            $null = [System.Windows.MessageBox]::Show(
                "Left margin must be $script:MIN_MARGIN-$script:MAX_MARGIN.", "Invalid", "OK", "Warning"
            )
            $null = $tLeft.Focus(); $tLeft.SelectAll(); return $false
        }
        $rv = ToInt $tRight.Text -1
        if ($rv -lt $script:MIN_MARGIN -or $rv -gt $script:MAX_MARGIN) {
            $null = [System.Windows.MessageBox]::Show(
                "Right margin must be $script:MIN_MARGIN-$script:MAX_MARGIN.", "Invalid", "OK", "Warning"
            )
            $null = $tRight.Focus(); $tRight.SelectAll(); return $false
        }
        $sv = ToDouble $tSpace.Text -1
        if ($sv -lt $script:MIN_SPACING -or $sv -gt $script:MAX_SPACING) {
            $null = [System.Windows.MessageBox]::Show(
                "Line spacing must be $script:MIN_SPACING-$script:MAX_SPACING.", "Invalid", "OK", "Warning"
            )
            $null = $tSpace.Focus(); $tSpace.SelectAll(); return $false
        }
        $script:UI.txtEditor.Margin = [System.Windows.Thickness]::new($lv, 0, $rv, 0)
        $script:State.BaseLineSpacing = [double]$sv
        ApplyLineSpacingToEditor
        $script:Settings.Editor.MarginLeft  = "$lv"
        $script:Settings.Editor.MarginRight = "$rv"
    ...
    $restoreOriginal = {
        ...
        $script:State.BaseLineSpacing = [double]$origSpacing
        ApplyLineSpacingToEditor
    }
    $bCancel.Add_Click({ & $restoreOriginal; $d.Close() })
    $d.Add_Closing({ param($s, $e)
...
#region ==================== ENCODING / EOL ====================
function script:SetEncoding([string]$Name) {
    $script:State.Encoding   = $Name
    $script:State.IsModified = $true
    try { $script:UI.txtEnc.Text = $Name } catch {}
    UpdateTitle
}

function script:SetLineEndingType([string]$Type) {
    $script:State.LineEnding = $Type
    $script:State.IsModified = $true
    try { $script:UI.txtEol.Text = EolDisplayLabel $Type } catch {}
    UpdateTitle
}
#endregion
...
function script:SetStatusBar([bool]$On) {
    ...
    $script:UI.statusBar.Visibility  = $vis
    $script:UI.statusBorder.Visibility = $vis
    if ($On) { UpdateStatusImmediate }
}
#endregion

#region ==================== WIRE EVENTS ====================

# Maximize bug fix: HwndSource hook for WM_GETMINMAXINFO (0x0024).
# Clamps the maximized window to the monitor's work area.
        ...
        $hook = [System.Windows.Interop.HwndSourceHook] {
            ...
            if ($msg -eq 0x0024) {  # WM_GETMINMAXINFO
                try {
                    ...
                    $hMon = [MaximizeHelper]::MonitorFromWindow($hwnd, $MONITOR_DEFAULTTONEAREST)
                    if ($hMon -eq [IntPtr]::Zero) { return [IntPtr]::Zero }

                    $miSize = 40
                    $miPtr = [System.Runtime.InteropServices.Marshal]::AllocHGlobal($miSize)
                    try {
                        # Zero-initialize the MONITORINFO structure
                        for ($i = 0; $i -lt $miSize; $i++) {
                            [System.Runtime.InteropServices.Marshal]::WriteByte($miPtr, $i, 0)
                        }
                        [System.Runtime.InteropServices.Marshal]::WriteInt32($miPtr, 0, $miSize)
                        if (-not [MaximizeHelper]::GetMonitorInfo($hMon, $miPtr)) { return [IntPtr]::Zero }

                        $workLeft   = [System.Runtime.InteropServices.Marshal]::ReadInt32($miPtr, 20)
                        $workTop    = [System.Runtime.InteropServices.Marshal]::ReadInt32($miPtr, 24)
                        $workRight  = [System.Runtime.InteropServices.Marshal]::ReadInt32($miPtr, 28)
                        $workBottom = [System.Runtime.InteropServices.Marshal]::ReadInt32($miPtr, 32)

                        $workW = $workRight  - $workLeft
                        $workH = $workBottom - $workTop

                        $borderPhys = 6 * $scale
                        $maxW = $workW + ($borderPhys * 2)
                        $maxH = $workH + ($borderPhys * 2)
                        $maxX = $workLeft - $borderPhys
                        $maxY = $workTop  - $borderPhys

                        # MINMAXINFO layout (each POINT = 8 bytes):
                        #   ptMaxSize      offset 8  (X=8,  Y=12)
                        #   ptMaxPosition  offset 16 (X=16, Y=20)
                        [System.Runtime.InteropServices.Marshal]::WriteInt32($lParam, 8,  [int]$maxW)
                        [System.Runtime.InteropServices.Marshal]::WriteInt32($lParam, 12, [int]$maxH)
                        [System.Runtime.InteropServices.Marshal]::WriteInt32($lParam, 16, [int]$maxX)
                        [System.Runtime.InteropServices.Marshal]::WriteInt32($lParam, 20, [int]$maxY)

                        if ($handled -is [System.Management.Automation.PSReference]) {
                            $handled.Value = $true
                        } else {
                            $handled = $true
                        }
                    } finally {
                        [System.Runtime.InteropServices.Marshal]::FreeHGlobal($miPtr)
                    }
                } catch {}
            }
            return [IntPtr]::Zero
...
# Menu events
 ...
 $script:UI.mnuDelete.Add_Click({
    if ($script:UI.txtEditor.SelectionLength -gt 0) { $script:UI.txtEditor.SelectedText = "" }
})
...
# Context menu events
 $script:UI.ctxCut.Add_Click({ $script:UI.txtEditor.Cut() })
 $script:UI.ctxCopy.Add_Click({ $script:UI.txtEditor.Copy() })
 $script:UI.ctxPaste.Add_Click({ $script:UI.txtEditor.Paste() })
 $script:UI.ctxSelAll.Add_Click({ $script:UI.txtEditor.SelectAll() })
...
# Format / View menus
 ...
 $script:UI.mnuZoomIn.Add_Click({ ZoomIn })
 $script:UI.mnuZoomOut.Add_Click({ ZoomOut })
 $script:UI.mnuZoomReset.Add_Click({ ZoomReset })
 $script:UI.mnuEolCRLF.Add_Click({ SetLineEndingType "CRLF" })
 $script:UI.mnuEolLF.Add_Click({ SetLineEndingType "LF" })
 $script:UI.mnuEolCR.Add_Click({ SetLineEndingType "CR" })
 $script:UI.mnuEncUtf8.Add_Click({ SetEncoding "UTF-8 (no BOM)" })
 $script:UI.mnuEncUtf8Bom.Add_Click({ SetEncoding "UTF-8 (BOM)" })
 $script:UI.mnuEncUtf16LE.Add_Click({ SetEncoding "UTF-16 LE (BOM)" })
 $script:UI.mnuEncUtf16BE.Add_Click({ SetEncoding "UTF-16 BE (BOM)" })
 ...
 $script:Window.Add_PreviewKeyDown({
    ...
    if ($ctrl -and -not $shift) {
        switch ($e.Key) {
            ...
            "D0"       { ZoomReset;    $e.Handled = $true }
            "NumPad0"  { ZoomReset;    $e.Handled = $true }
        }
...
# Smooth pixel-based scrolling
 $script:UI.txtEditor.Add_PreviewMouseWheel({
    param($sender, $e)
    # Ctrl+Wheel = zoom
    if (([System.Windows.Input.Keyboard]::Modifiers -band "Control") -ne 0) {
        if ($e.Delta -gt 0) { ZoomIn } else { ZoomOut }
        $e.Handled = $true
        return
    }
    if ($null -eq $script:EditorScrollViewer) { return }
    $e.Handled = $true

    $linesPerDelta = $script:SCROLL_LINES_PER_DELTA / $script:WHEEL_DELTA
    $lineHeight = $script:UI.txtEditor.GetValue([System.Windows.Controls.TextBlock]::LineHeightProperty)
    if ($lineHeight -le 0) { $lineHeight = $script:UI.txtEditor.FontSize * 1.2 }

    $pixelsToScroll = $e.Delta * $linesPerDelta * $lineHeight
    $newOffset = $script:EditorScrollViewer.VerticalOffset - $pixelsToScroll

    # Clamp to valid range
    $maxOffset = $script:EditorScrollViewer.ScrollableHeight
    if ($newOffset -lt 0)        { $newOffset = 0 }
    if ($newOffset -gt $maxOffset){ $newOffset = $maxOffset }

    $script:EditorScrollViewer.ScrollToVerticalOffset($newOffset)
})
 ...
 $script:Window.Add_Loaded({
    ...
    UpdateZoomDisplay
    UpdateZoomMenuState
    # Cache the internal ScrollViewer for custom scroll speed logic
    $script:EditorScrollViewer = GetScrollViewer $script:UI.txtEditor
    $null = $script:UI.txtEditor.Focus()
})
 ...
 $script:Window.Add_Closing({
    ...
    try { $script:StatusUpdateTimer.Stop() } catch {}
    SaveSettings
})
#endregion
